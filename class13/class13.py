...
# 建立字典 :使用大括號 {} ,key 和 value 之間用 : 隔開
d = {"蘋果": 20, "香蕉": 30, "橘子": 25}
print(d)

# 從字典中取的特定 key 對應的 value
# 如果 key 不存在會產生 KeyError 錯誤
d = {"蘋果": 20, "香蕉": 30, "橘子": 25}
print(d["蘋果"])
# print(d["梨子"]) # 這行會產生 KeyError '梨子' 錯誤

# 覑利字典 : 有多種方式可以走訪字典中的資料
d = {"蘋果": 20, "香蕉": 30, "橘子": 25}
...
for v in d.values():
    print(v)  # 直接印出 value, 但不知道對應的 key 是什麼
...
# 新增/修改字典的內容
# 直接指定 key 對應的 value, 如果 key 已存在就是修改,如果key 不存在則是新增
d = {"蘋果": 20, "香蕉": 30, "橘子": 25}
...
d = {"蘋果": 20, "香蕉": 30, "橘子": 25}
d.pop("蘋果")  # 刪除蘋果
# 如果要刪掉的key不存在,會出現 KeyError ,所以可以加上第二個參數,當key不存在時,不會有任何變化
...
print("梨子" in d)  # False,梨子這個 key 不存在


# 檢查某個元素是否在list中
# 使用in 可以快速檢查某個元素是否在list中
L = [1, 2, 3, 4, 5]
print(3 in L)  # True,3 在 L 中
